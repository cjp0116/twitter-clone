...
export function PollDisplay({ tweetId, currentUserId }: PollDisplayProps) {
  ...
  useEffect(() => {
    fetchPollData();
    const channel = supabase.channel(`poll-${tweetId}`)
    .on(
      "postgres_changes",
      {
        event: "*",
  ...
  const fetchPollData = async () => {
    try {
      ...
      if (optionsError || !optionsData) return;

      setOptions(optionsData || [])

      setTotalVotes(optionsData.reduce((acc, option) => acc + option.vote_count, 0));
  ...
  const leadingOption = options.find(opt => opt.id === leadingOptionId) || null;
  const leadingOptionPercentage = leadingOption ? getPercentage(leadingOption.vote_count) : 0;
