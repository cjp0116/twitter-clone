...
CREATE POLICY "blocks_insert_own" ON public.blocks FOR
...
CREATE POLICY "mutes_select_own" ON public.mutes FOR
SELECT USING (auth.uid() = muter_id);
-- Users can only create their own mutes
CREATE POLICY "mutes_insert_own" ON public.mutes FOR
INSERT WITH CHECK (auth.uid() = muter_id);
...
CREATE OR REPLACE FUNCTION is_blocked(blocker_user_id UUID, blocked_user_id UUID) RETURNS BOOLEAN AS $$ BEGIN RETURN EXISTS (
    SELECT 1
    FROM public.blocks
...
CREATE OR REPLACE FUNCTION is_muted(muter_user_id UUID, muted_user_id UUID) RETURNS BOOLEAN AS $$ BEGIN RETURN EXISTS (
    SELECT 1
    FROM public.mutes
...
CREATE OR REPLACE FUNCTION handle_block_unfollow() RETURNS TRIGGER AS $$ BEGIN -- Remove follow relationship in both directions
...
AFTER
INSERT ON public.blocks FOR EACH ROW EXECUTE FUNCTION handle_block_unfollow();
-- Grant permissions
GRANT SELECT,
  INSERT,
  DELETE ON public.blocks TO authenticated;
GRANT SELECT,
  INSERT,
  DELETE ON public.mutes TO authenticated;