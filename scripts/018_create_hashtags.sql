...
CREATE POLICY "Anyone can view tweet_hashtags"
  ON public.tweet_hashtags
  FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Users can insert tweet_hashtags for their tweets"
  ON public.tweet_hashtags
  FOR INSERT
  TO authenticated
  WITH CHECK (
    EXISTS (
      ...
      WHERE id = tweet_id AND author_id = auth.uid()
    )
  );

CREATE POLICY "Users can delete tweet_hashtags for their tweets"
  ON public.tweet_hashtags
  FOR DELETE
  TO authenticated
  USING (
...
BEGIN
  UPDATE public.hashtags
  SET
    tweet_count = GREATEST(0, tweet_count - 1),
    updated_at = NOW()
...
CREATE TRIGGER trigger_increment_hashtag_count
  AFTER INSERT ON public.tweet_hashtags
  FOR EACH ROW
...
CREATE TRIGGER trigger_decrement_hashtag_count
  AFTER DELETE ON public.tweet_hashtags
  FOR EACH ROW
  EXECUTE FUNCTION decrement_hashtag_count();

-- Grant permissions
GRANT SELECT, INSERT, UPDATE ON public.hashtags TO authenticated;
GRANT SELECT, INSERT, DELETE ON public.tweet_hashtags TO authenticated;
