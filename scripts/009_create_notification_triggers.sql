...
BEGIN
  ...
  IF tweet_author_id != NEW.user_id THEN
    ...
    VALUES (tweet_author_id, NEW.user_id, 'like', NEW.tweet_id);
  END IF;
...
BEGIN
  -- Only create notification for retweets (not original tweets)
  IF NEW.retweet_of_id IS NOT NULL THEN
    ...
    IF original_author_id != NEW.author_id THEN
      INSERT INTO public.notifications (user_id, actor_id, type, tweet_id)
      VALUES (original_author_id, NEW.author_id, 'retweet', NEW.id);
    END IF;
  END IF;
...
BEGIN
  ...
  IF NEW.reply_to_id IS NOT NULL THEN
    -- Get the author of the parent tweet
