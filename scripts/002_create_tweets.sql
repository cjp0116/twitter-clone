...
create policy "tweets_select_all" on public.tweets for
select using (true);
create policy "tweets_insert_own" on public.tweets for
insert with check (auth.uid() = author_id);
create policy "tweets_update_own" on public.tweets for
